...
public class ChatEndpoint {
    ...
    private static final Map<String, Set<Session>> conversations = new ConcurrentHashMap<>();
    ...
    public void onOpen(Session session, @PathParam("conversationId") String conversationId) {
        ...
        conversations.computeIfAbsent(conversationId, k -> ConcurrentHashMap.newKeySet()).add(session);
    ...
    public void onMessage(String message, Session session) throws IOException {
        ...
        Set<Session> clients = conversations.getOrDefault(conversationId, Collections.emptySet());
        ...
        for (Session client : clients) {
            if (client.isOpen() && !client.equals(session)) { 
                try {
                    ...
                    log.info("Failed to send to client " + client.getId() + ": " + e.getMessage());
                }
            }
        }
    }
    ...
    public void onClose(Session session) {
        String conversationId = (String) session.getUserProperties().get("conversationId");
        if (conversationId != null) {
            Set<Session> sessions = conversations.get(conversationId);
            if (sessions != null) {
    ...
    public void onError(Session session, Throwable throwable) {
        log.info("WS Error for session " + session.getId());
